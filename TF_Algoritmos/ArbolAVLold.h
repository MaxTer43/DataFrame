...
enum Recorrer { EnOrden2, EnOrdenEdad2, EnOrdenInverso2, PreOrden2, PostOrden2, Especifico2, Mayor2, Menor2 };
...
public:
	...
	void NOempezarCon(T valor) {
		this->raiz = NOempezarCon(raiz, valor);
	}
	void terminarCon(T valor) {
		this->raiz = terminarCon(raiz, valor);
	}
	void mostrar(Recorrer recorrido) {
		switch (recorrido) {
		case EnOrden2: enOrden(raiz); break;
		case EnOrdenInverso2: enOrdenInverso(raiz); break;
		case PreOrden2: preOrden(raiz); break;
		case PostOrden2: postOrden(raiz); break;
...
private:
	...
	Nodo* buscar(Nodo* nodo, T valor)
	{
		...
		if (nodo != NULL)
		{
			...
			//cout << raiz->valor << endl << " (" << raiz->altura << ") " << endl;

			if (valor == raiz->valor)
				cout << raiz->valor << endl << endl;
	...
	Nodo* NOempezarCon(Nodo* raiz, T valor)
	{
		if (raiz != NULL)
		{
			NOempezarCon(raiz->hizq, valor);
			//cout << raiz->valor << endl << " (" << raiz->altura << ") " << endl;

			if (raiz->valor.rfind(valor, 0) != 0) {
				cout << raiz->valor << endl;
			}
			NOempezarCon(raiz->hder, valor);
		}
		return raiz;
	}
